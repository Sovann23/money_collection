...
export function ContributionForm({ showToast, editingContribution, setEditingContribution }) {
  ...
  /**
   * Toggle helper: if the user clicks the already-active value, clear it back to ''.
   * Otherwise set the new value normally.
   */
  ...
  const inputCls =
    'w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/60 text-gray-900 dark:text-gray-100 placeholder-gray-400 text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all'

  /**
   * Option button styling.
   * active  → filled accent colour
   * inactive → neutral, with a subtle hover ring
   * The cursor always shows "pointer" so the user knows it's clickable.
   */
  const optionBtn = (active, green) =>
    `flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border-2 text-sm font-semibold cursor-pointer transition-all select-none
    ${active
      ? green
        ? 'bg-emerald-500 border-emerald-500 text-white shadow-sm'
        : 'bg-blue-500 border-blue-500 text-white shadow-sm'
      : 'bg-gray-50 dark:bg-gray-700/60 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10'
    }`
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Payment method — clicking active button deselects it */}
        <div>
          <label className="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-1.5">
            {t.paymentMethod}
          </label>
        ...
        {/* Currency — clicking active button deselects it */}
        <div>
          <label className="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-1.5">
            {t.currency}
          </label>
        ...
        <div className="flex gap-2 pt-1">
          <button
            type="submit"
            className="flex-1 py-2.5 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-xl text-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
          >
          ...
          {editingContribution && (
            <button
              type="button"
              onClick={() => { setEditingContribution(null); setForm(EMPTY) }}
